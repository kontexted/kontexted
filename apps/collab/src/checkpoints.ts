...
export const checkNeedsReseed = async (roomName: string): Promise<boolean> => {
  ...
  if (roomLastSavedAt === null) {
    console.log(`[collab] DB has content for ${roomName}: DB=${dbUpdatedAt.toISOString()} vs Room=null`);
    return true;
  }

  if (dbUpdatedAt.getTime() > roomLastSavedAt.getTime()) {
    console.log(`[collab] DB has newer content for ${roomName}: DB=${dbUpdatedAt.toISOString()} vs Room=${roomLastSavedAt.toISOString()}`);
    return true;
  }
...
export const reseedDocumentFromDb = async (roomName: string): Promise<void> => {
  ...
  console.log(`[collab] Reseeding document from DB: ${roomName}`);
